...
@WebServlet(name = "AddProductToCartServlet")
        ...
        if(productId > 0 && productAmount.compareTo(BigDecimal.ZERO)  ==1 && user != null) {
            ...
            if (byUserId.isPresent()) {
                ...
                if (productInCart.isPresent()) {
                    //produkt juz jest w koszyku
                    productInCart.get().setAmount(productInCart.get().getAmount().add(productAmount));
                    CartRepository.saveCart(cart);
                    writer.write("Dodano produkut z id "+productId+" do koszyka");
                } else {
                    //tego produktu nie ma w koszyku
            ...
            if(productAmount.compareTo(BigDecimal.ZERO) <= 0){
                writer.write("Nie można dodać produktu z ilością ujemną bądź zerową");
